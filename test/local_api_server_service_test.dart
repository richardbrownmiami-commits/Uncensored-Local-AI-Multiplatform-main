...
void main() {
  ...
  test('init starts the localhost server when enabled', () async {
    final port = await _freePort();
    final storage = Get.find<ChatStorageService>();
    storage.localApiServerPort = port;
  ...
  test('starts, reports health, and returns an OpenAI model list shape', () async {
    final port = await _freePort();
  ...
  test('chat completions return OpenAI-style error when no model is loaded', () async {
    final port = await _freePort();
    await apiServer.start(requestedPort: port);
