...
import '../widgets/github_confirm_dialog.dart';
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Worker? _pendingActionWorker;

  @override
  void initState() {
    super.initState();
    _scrollController.addListener(_handleChatScroll);
    // Whenever the model proposes a GitHub write, this is the single place
    // that surfaces it — as a blocking approve/cancel dialog, never auto-run.
    _pendingActionWorker = ever(_chatCtrl.pendingAction, (action) {
      if (action != null && mounted) {
        GithubActionConfirmDialog.maybeShow(context, action);
      }
    });
  }
