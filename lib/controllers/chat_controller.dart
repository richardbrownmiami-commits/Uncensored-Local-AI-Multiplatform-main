...
class ChatController extends GetxController {
  ...
  /// Set when the model's last reply contained a GitHub write request.
  ...
  /// Result of the most recent read-only tool call (read file / list
  /// issues), shown inline in the chat. Null means nothing to show.
  ...
  Future<void> sendMessage(String text, {String? modelFilename}) async {
    ...
    try {
      final baseSystemPrompt = chat.systemPrompt.isNotEmpty
          ? chat.systemPrompt
          : systemPrompt.value;
      final effectiveSystemPrompt = _github.isConfigured
      ...
      // Detect a GitHub tool-call request in what the model just produced.
      // Read-only actions run immediately and their result is appended to
      // the same message. A write request is never executed here — it is
      // only surfaced via pendingAction for the UI to show an approve/
      // cancel dialog; GitHubService.commitFile() is called solely from
      // that confirmation handler (see approvePendingAction below).
  ...
  /// dialog. This is the single place in the app that is allowed to call
  /// GitHubService.commitFile().
  Future<String?> approvePendingAction() async {
    final action = pendingAction.value;
    if (action == null || action.kind != ToolKind.writeFile) return null;
